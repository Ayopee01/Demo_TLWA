...
function PaymentPopup({ open, amount, discount, onClose, onSubmit, paymentMethod, setPaymentMethod }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!paymentMethod) return setError("กรุณาเลือกวิธีโอนเงิน");
    if (!file) return setError("กรุณาอัปโหลดสลิปโอนเงิน");
    setError("");
    onSubmit({ file, paymentMethod });
  };
